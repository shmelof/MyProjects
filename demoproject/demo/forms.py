...
def validate_password_len(password):
    if len(password) < 6:
        raise ValidationError('Длинна пароля не может быть меньше 6 символов')


class RegisterUserForm(forms.ModelForm):
    username = forms.CharField(label="Логин",
                               validators=[RegexValidator('^[a-zA_Z0-9-]+$',
    ...
    def clean(self):
        ...
        if password and password2 and password != password2:
            raise ValidationError({
                'password2': ValidationError('Веденные пароли не совпадают', code='password_mismatch')
            })
